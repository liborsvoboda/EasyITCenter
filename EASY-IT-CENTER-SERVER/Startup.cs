...
namespace EasyITCenter {
    ...
    public class Startup {
        ...
        public void Configure(IApplicationBuilder app, IHostApplicationLifetime serverLifetime) {
            ...
            app.Use(async (context, next) => { context.Response.StatusCode = StatusCodes.Status200OK; ServerModuleAndServiceList? serverModule = null;

                /* Login Request Detected ON PHYSICAL MODULE - Ignored */ string requestedModulePath = null;
                ...
                if (requestedModulePath == "correct") { try { context.Response.Cookies.Delete("IsLoginRequest"); context.Response.Cookies.Delete("RequestedModulePath"); } catch { } await next(); }
                    ...
                    serverModule = new EasyITCenterContext().ServerModuleAndServiceLists.FirstOrDefault(a => a.UrlSubPath.ToLower().Replace("/", "").StartsWith(context.Request.Path.Value.ToLower().Replace("/", "")));
            ...
            app.Use(async (context, next) => { await next();
                if (context.Response.StatusCode == StatusCodes.Status404NotFound && !context.Request.Path.ToString().Split("/").Last().Contains(".")) { 
                    ...
                    if (isLoginRequest == "correct") { context.Request.Path = BackendServer.ServerRuntimeData.SpecialUserWebRootPath; context.Response.StatusCode = StatusCodes.Status200OK; 
                        ...
                        requstedModule = new EasyITCenterContext().ServerModuleAndServiceLists.FirstOrDefault(a => a.UrlSubPath.ToLower().Replace("/", "").StartsWith(requestedModulePath.ToLower().Replace("/", "")));
