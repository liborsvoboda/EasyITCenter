...
async function DownloadApiFiles(onlyReload) {
	...
	var def = $.ajax({
		global: false, type: "GET", url: "/GLOBALNETDocumentationList/byGroup/" + ConfigApiGroupId, dataType: 'json',
...
async function StartupModeSelection(){
	...
	try {
		...
		if (1 == 0 && Metro.storage.getItem('ApiToken', null) != null) {
			...
			await axios.get(window.location.origin + "/WebApi/WebDocumentation/MdPreviewFile",{
		 	headers: { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, POST, PUT" }
			}).then((response) => {
				console.log("Editor1", response);
...
function UpdateFileMenu() {
	...
	DocFiles.forEach(file => {
		...
		html += "<li id='" + file.filename + "' class='pt-2 pb-2 text-bold pl-3' onclick='FileMenuChange(this.id)' >" + file.filename + "</li>"; 
	});
	html += "</ul>";
