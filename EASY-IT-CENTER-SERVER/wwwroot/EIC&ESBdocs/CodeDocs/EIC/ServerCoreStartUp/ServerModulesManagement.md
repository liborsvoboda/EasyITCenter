...
namespace EasyITCenter.ServerCoreConfiguration {
    ...
    public class ServerModules {
        ...
        internal static void ConfigureMarkDocs(ref IServiceCollection services) {
            if (ServerConfigSettings.GitServerEnabled) {
                services.AddMarkDocs(config => {
                    config.FromStaticDirectory(
                        Path.GetFullPath(Path.Combine(ServerRuntimeData.Startup_path, "CodeDocs")),
                        //@"d:\crex\techdocs",
                    ...
                    config.UseMarkdown(markdown => {
                        ...
                        markdown.AddHighlightJs(Path.Combine(ServerRuntimeData.Startup_path, "CodeDocs", "hljs"));
                        markdown.AddPlantUmlWebService();
                    });

                    config.UseDiskCache(Path.Combine(ServerRuntimeData.Startup_path, "CodeDocs", "cache"));

                    config.AddTags();
                    config.AddSearch(Path.Combine(ServerRuntimeData.Startup_path, "CodeDocs", "search"));
