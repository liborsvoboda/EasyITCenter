    ...
    public class GLOBALNETIncomingInvoiceListApi : ControllerBase {
        ...
        public async Task<string> InsertIncomingInvoiceList([FromBody] IncomingInvoiceList record) {
            try {
                ...
                documentAdvice = new EASYDATACenterContext().SystemDocumentAdviceLists.Where(a => a.DocumentType == "incomingInvoice" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
