    ...
    public class GLOBALNETIncomingOrderListApi : ControllerBase {
        ...
        public async Task<string> InsertIncomingOrderList([FromBody] IncomingOrderList record) {
            try {
                ...
                documentAdvice = new EASYDATACenterContext().SystemDocumentAdviceLists.Where(a => a.DocumentType == "incomingOrder" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
