...
namespace EASYDATACenter.Controllers {
    ...
    [ApiController]
    [Route("GLOBALNETSolutionFailList")]
    public class GLOBALNETSolutionFailListApi : ControllerBase {

        [HttpGet("/GLOBALNETSolutionFailList")]
        ...
        public async Task<string> InsertSolutionFailList([FromBody] SolutionFailList record) {
            try {
                ...
                var data = new EASYDATACenterContext().SolutionFailLists.Add(record);
        ...
        [HttpPost("/GLOBALNETSolutionFailList")]
        [Consumes("application/json")]
        public async Task<string> UpdateSolutionFailList([FromBody] SolutionFailList record) {
            try {
                var data = new EASYDATACenterContext().SolutionFailLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { InsertedId = record.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { Status = DBResult.error.ToString(), RecordCount = result, ErrorMessage = string.Empty });
        ...
        [HttpDelete("/GLOBALNETSolutionFailList/{id}")]
        [Consumes("application/json")]
        public async Task<string> DeleteSolutionFailList(string id) {
            try {
                ...
                var data = new EASYDATACenterContext().SolutionFailLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { InsertedId = record.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { Status = DBResult.error.ToString(), RecordCount = result, ErrorMessage = string.Empty });
            } catch (Exception ex) {
