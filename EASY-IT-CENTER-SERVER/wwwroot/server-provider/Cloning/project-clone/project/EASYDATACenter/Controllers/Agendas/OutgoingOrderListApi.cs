    ...
    public class GLOBALNETOutgoingOrderListApi : ControllerBase {
        ...
        public async Task<string> InsertOutgoingOrderList([FromBody] OutgoingOrderList record) {
            try {
                ...
                documentAdvice = new EASYDATACenterContext().SystemDocumentAdviceLists.Where(a => a.DocumentType == "outgoingOrder" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
