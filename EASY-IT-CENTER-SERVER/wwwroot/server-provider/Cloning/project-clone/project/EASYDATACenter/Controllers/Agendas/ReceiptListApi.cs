    ...
    public class GLOBALNETReceiptListApi : ControllerBase {
        ...
        public async Task<string> InsertReceiptList([FromBody] ReceiptList record) {
            try {
                ...
                documentAdvice = new EASYDATACenterContext().SystemDocumentAdviceLists.Where(a => a.DocumentType == "receipt" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
                ...
                //Load Receipt Status
                BusinessPaymentStatusList receiptStatus = new EASYDATACenterContext().BusinessPaymentStatusLists.Where(a => a.Receipt).FirstOrDefault();
