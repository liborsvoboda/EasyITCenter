...
(function () {
    ...
    const createSolutionMixedEnumList = (editor, callback) => {
      return () => {
        const SolutionMixedEnumList = getTemplates(editor);
        if (isFunction(SolutionMixedEnumList)) {
          SolutionMixedEnumList(callback);
        } else if (isString(SolutionMixedEnumList)) {
          fetch(SolutionMixedEnumList).then(res => {
            if (res.ok) {
              res.json().then(callback);
            }
          });
        } else {
          callback(SolutionMixedEnumList);
        }
      };
    };
    ...
    const open = (editor, SolutionMixedEnumList) => {
      const createTemplates = () => {
        if (!SolutionMixedEnumList || SolutionMixedEnumList.length === 0) {
        ...
        return Optional.from(global$2.map(SolutionMixedEnumList, (template, index) => {
    ...
    const register$1 = editor => {
      ...
      editor.addCommand('mceTemplate', createSolutionMixedEnumList(editor, showDialog(editor)));
    };
