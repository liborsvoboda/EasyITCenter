...
namespace EasyITCenter.ServerCoreStructure {
    ...
    public class StaticFilesFileProviderService : IFileProvider {
        ...
        public StaticFilesFileProviderService(IServiceProvider serviceProvider) {
            this.serviceProvider = serviceProvider;
        }

        public IDirectoryContents GetDirectoryContents(string subpath) {
            using (var scope = CreateScope()) {
                return new DirectoryContents(scope.ServiceProvider.GetRequiredService<StaticFileDbService>().GetFiles(subpath).ToArray());
            }
        }

        public IFileInfo GetFileInfo(string subpath) {
            using (var scope = CreateScope()) {
                var find = scope.ServiceProvider.GetRequiredService<StaticFileDbService>().ReadFile(subpath);
