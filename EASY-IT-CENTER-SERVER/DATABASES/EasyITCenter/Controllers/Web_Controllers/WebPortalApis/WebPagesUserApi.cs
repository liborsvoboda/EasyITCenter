    ...
    public class WebPagesUserApi : ControllerBase {
        ...
        public async Task<string> PostRegistration([FromBody] WebRegistration record) {
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    origUser = new EasyITCenterContext().SolutionUserLists.Where(a => a.UserName == record.EmailAddress).FirstOrDefault();
                }
                if (origUser != null) {
        ...
        public async Task<string> GetWebUser(string language = "cz") {
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    result = new EasyITCenterContext().SolutionUserLists.Where(a => a.Id == int.Parse(authId)).First();
                }
