    ...
    public class EasyITCenterStoredProceduresListApi : ControllerBase {
        ...
        public async Task<string> GetSystemSpGetTableList() {
            try {
                ...
                data = new EasyITCenterContext().EasyITCenterCollectionFromSql<GenericObject>("EXEC SystemSpGetTableList;");
        ...
        public async Task<string> GetSystemTableList() {
            try {
                List<GenericObject> data = new();
                data = new EasyITCenterContext().EasyITCenterCollectionFromSql<GenericObject>("EXEC SystemSpGetSystemPageList;");
                return JsonSerializer.Serialize(data, new JsonSerializerOptions() { ReferenceHandler = ReferenceHandler.IgnoreCycles, WriteIndented = true });
            } catch (Exception ex) {
                return JsonSerializer.Serialize(new DBResultMessage() { Status = DBResult.error.ToString(), RecordCount = 0, ErrorMessage = DataOperations.GetUserApiErrMessage(ex) });
            }
