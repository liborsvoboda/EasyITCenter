...
namespace EasyITCenter.Controllers {
    ...
    public class WebGlobalMessageModuleApi : ControllerBase {
        ...
        public WebGlobalMessageModuleApi(Microsoft.AspNetCore.Hosting.IHostingEnvironment hostingEnvironment) {
        ...
        public async Task<string> SaveNewsletterPreview([FromBody] object HtmlContent) {
            try {
                string data;
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    data = new EasyITCenterContext().WebSettingLists.Where(a => a.Key == "NewsletterPreviewTemplate").First().Value;
                }
                ...
                System.IO.File.WriteAllText(Path.Combine(ServerRuntimeData.Startup_path, "wwwroot", "ServerCoreTools","systemTools", "newsletter-preview", "index.html"), data);
...
//TODO PO VYTVORENI USR SUB PARAMS
/*
        ...
        public async Task<string> GetPrivateMessageList(bool archived) {
            ...
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    data = new EasyITCenterContext().SolutionMessageModuleLists
        ...
        public async Task<string> GetUnreadPrivateMessageCount(bool archived) {
            ...
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    unreadPrivateMessages = new EasyITCenterContext().MessageModuleLists.Where(a => a.MessageType.Name == "private" 
        ...
        public async Task<string> GetReservationMessageList(bool archived) {
            ...
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    data = new EasyITCenterContext().MessageModuleLists.Where(a => a.MessageType.Name == "reservation" && a.GuestId == int.Parse(authId)
        ...
        public async Task<IActionResult> SetPrivateMessageAnswer([FromBody] WebPrivateMessage messageAnswer) {
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    parentMessage = new EasyITCenterContext().MessageModuleLists.Where(a => a.Id == messageAnswer.ParentId && a.GuestId == int.Parse(authId) && a.MessageType.Name == "private").FirstOrDefault();
                }
                if (parentMessage != null) {
                    ...
                    var data = new EasyITCenterContext().MessageModuleLists.Add(answerMessage);
        ...
        public async Task<IActionResult> ArchivePrivateMessage(int messageId, string language = "cz") {
            ...
            try {
                string authId = User.FindFirst(ClaimTypes.PrimarySid.ToString()).Value;
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    archiveMessage = new EasyITCenterContext().MessageModuleLists.Where(a => ( a.Id == messageId && a.MessageParentId == null) || ( a.MessageParentId == messageId)
                ...
                if (archiveMessage != null) {
                    ...
                    var data = new EasyITCenterContext();
        ...
        public async Task<IActionResult> SetShownPrivateMessage(int messageId, string language = "cz") {
            ...
            try {
                string authId = User.FindFirst(ClaimTypes.PrimarySid.ToString()).Value;
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    archiveMessage = new EasyITCenterContext().MessageModuleLists.Where(a => a.Id == messageId && a.GuestId == int.Parse(authId) && !a.Shown && a.MessageType.Name == "private").FirstOrDefault();
                }
                if (archiveMessage != null) {
                    archiveMessage.Shown = true;
                    var updateMessage = new EasyITCenterContext().MessageModuleLists.Update(archiveMessage);
        ...
        public async Task<string> GetDiscussionForumList(bool archived) {
            ...
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    data = new EasyITCenterContext().MessageModuleLists
        ...
        public async Task<IActionResult> SetDiscussionContribution([FromBody] WebDiscussionContribution contribution) {
            try {
                ...
                MessageModuleList parentMessage;
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    parentMessage = new EasyITCenterContext().MessageModuleLists.Where(a => a.Id == contribution.ParentId && a.MessageType.Name == "discussionForum").FirstOrDefault();
                }
                if (parentMessage != null) {
                    MessageModuleList answerMessage = new() {
