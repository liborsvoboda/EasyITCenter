...
namespace EasyITCenter.Pages {
    ...
    public partial class BusinessWarehouseListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                DgListView.ItemsSource = await CommApi.GetApiRequest<List<BusinessWarehouseList>>(ApiUrls.EasyITCenterBusinessWarehouseList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
            } catch (Exception autoEx) { App.ApplicationLogging(autoEx); }
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommApi.DeleteApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommApi.PutApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommApi.PostApiRequest(ApiUrls.EasyITCenterBusinessWarehouseList, httpContent, null, App.UserData.Authentification.Token); }
