...
namespace EASYDATACenter.Controllers {
    ...
    [Authorize]
    [ApiController]
    [Route("LicenseSHOPERIncomingInvoiceList")]
    public class LicenseSHOPERIncomingInvoiceListApi : ControllerBase {

        [HttpGet("/LicenseSHOPERIncomingInvoiceList")]
        public async Task<string> GetIncomingInvoiceList() {
            List<IncomingInvoiceList> data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted //with NO LOCK
            })) {
                data = new LicenseSHOPERContext().IncomingInvoiceLists.ToList();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpGet("/LicenseSHOPERIncomingInvoiceList/Filter/{filter}")]
        public async Task<string> GetIncomingInvoiceListByFilter(string filter) {
            List<IncomingInvoiceList> data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted //with NO LOCK
            })) {
                data = new LicenseSHOPERContext().IncomingInvoiceLists.FromSqlRaw("SELECT * FROM IncomingInvoiceList WHERE 1=1 AND " + filter.Replace("+", " ")).AsNoTracking().ToList();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpGet("/LicenseSHOPERIncomingInvoiceList/{id}")]
        public async Task<string> GetIncomingInvoiceListKey(int id) {
            IncomingInvoiceList data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted
            })) {
                data = new LicenseSHOPERContext().IncomingInvoiceLists.Where(a => a.Id == id).First();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpPut("/LicenseSHOPERIncomingInvoiceList")]
        ...
        public async Task<string> InsertIncomingInvoiceList([FromBody] IncomingInvoiceList record) {
            try {
                ...
                documentAdvice = new LicenseSHOPERContext().DocumentAdviceLists.Where(a => a.DocumentType == "incomingInvoice" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
                ...
                var data = new LicenseSHOPERContext().IncomingInvoiceLists.Add(record);
        ...
        [HttpPost("/LicenseSHOPERIncomingInvoiceList")]
        [Consumes("application/json")]
        public async Task<string> UpdateIncomingInvoiceList([FromBody] IncomingInvoiceList record) {
            try {
                var data = new LicenseSHOPERContext().IncomingInvoiceLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
        ...
        [HttpDelete("/LicenseSHOPERIncomingInvoiceList/{id}")]
        [Consumes("application/json")]
        public async Task<string> DeleteIncomingInvoiceList(string id) {
            try {
                ...
                docNumber = new LicenseSHOPERContext().IncomingInvoiceLists.First(a => a.Id == int.Parse(id)).DocumentNumber;
                IncomingInvoiceList record = new() { Id = int.Parse(id), DocumentNumber = docNumber };

                var data = new LicenseSHOPERContext().IncomingInvoiceLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
