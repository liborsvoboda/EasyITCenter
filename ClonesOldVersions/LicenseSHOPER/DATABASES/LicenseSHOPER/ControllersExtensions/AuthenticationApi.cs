...
namespace EASYDATACenter.ControllersExtensions {
    ...
    [ApiController]
    [Route("LicenseSHOPERAuthentication")]
    public class LicenseSHOPERAuthenticationApi : ControllerBase {
        private static Encoding ISO_8859_1_ENCODING = Encoding.GetEncoding("ISO-8859-1");

        [AllowAnonymous]
        [HttpPost("/LicenseSHOPERAuthentication")]
        public IActionResult Authenticate([FromHeader] string Authorization) {
            ...
            if (user == null)
                return BadRequest(new { message = ServerCoreDbOperations.DBTranslate("UsernameOrPasswordIncorrect", ServerConfigSettings.SpecialServerLanguage) });

            if (!ServerConfigSettings.ServerTimeTokenValidationEnabled) { user.Expiration = null; }
        ...
        public static AuthenticateResponse? Authenticate(string? username, string? password) {
            ...
            var user = new LicenseSHOPERContext()
        ...
        public static bool RefreshUserToken(string username, AuthenticateResponse token) {
            try {
                var dbUser = new LicenseSHOPERContext()
                ...
                var data = new LicenseSHOPERContext().UserLists.Update(dbUser);
