...
namespace EASYDATACenter.ControllersExtensions {
    ...
    [ApiController]
    [Route("SHOPINGERAuthentication")]
    public class SHOPINGERAuthenticationApi : ControllerBase {
        private static Encoding ISO_8859_1_ENCODING = Encoding.GetEncoding("ISO-8859-1");

        [AllowAnonymous]
        [HttpPost("/SHOPINGERAuthentication")]
        ...
        public static bool RefreshUserToken(string username, AuthenticateResponse token) {
            try {
                var dbUser = new SHOPINGERContext()
                ...
                var data = new SHOPINGERContext().UserLists.Update(dbUser);
