    ...
    public class SHOPINGERItemListApi : ControllerBase {
        ...
        public async Task<string> DeleteItemList(string id) {
            try {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) { Attachmentdata = new SHOPINGERContext().AttachmentLists.Where(a => a.ParentType == "ITEM" && a.ParentId == int.Parse(id)).ToList(); }
                SHOPINGERContext itemData = new SHOPINGERContext(); itemData.AttachmentLists.RemoveRange(Attachmentdata);
